...
config_model = {
    ...
    "grid": {
        "type": "structured",
        "buffer": True,
        "boundary": os.path.join("input", "vector", "boundary2.shp"),
        "cell_size": 200,
        "nlay": 1,
    ...
    #     ]},
    #     "chd": {0: [
    #         {"head": os.path.join("input", "raster", "river_stages_dz.tif"), "layers": [1],
    #          "geometry": os.path.join("input", "vector", "zones_river.shp")},
    #     ]},
    #     # "wel": {0: [
    #     #     {"geometry": os.path.join("input", "vector", "intakes.shp"), "q": "q_1", "layers": "lay"},
    #     # ]}
    # },
    # # "parameters": {
    # #     "npf": {"k": [5], "icelltype": [1]},
...
a.export()
# import pandas as pd
# obs_prp = pd.read_csv(os.path.join("dzerginsk", "input", "vector", "well_regime_new.csv"))
# obs_prp["well"] = "H_" + obs_prp["name"].astype(str)
# obs_prp["time"] = 1.0
# obs_res = pd.read_csv(os.path.join("dzerginsk", "output", "residuals.csv"))
...
# #
# table_obs.to_csv(os.path.join("dzerginsk", "dzerginsk.obs.head.csv"), index=False)
#
# import numpy as np
...
#
# boundary = gpd.read_file(os.path.join("dzerginsk", "input", "vector", "boundary2.shp"))
# polygon = boundary.geometry.values[0]
# points = pd.read_csv(os.path.join("dzerginsk", "input", "vector", "well_regime.csv"))
...
# points_in_polygon.to_file(os.path.join("dzerginsk", "input", "vector", 'pp.shp'))